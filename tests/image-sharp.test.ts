...
import { getBufferImages, getGridTiles, getHorizontalTiles, getImage, getSize, getUniqueGridTiles, getUniqueHorizontalTiles, getUniqueVerticalTiles, getVerticalTiles, writeImages } from '../src/image-sharp'
...
test.skip.concurrent('getHorizontalTiles', async () => {
  ...
  await Promise.all(tiles.map(async (tile, idx) => {
    const filename = path.join(IMG_FOLDER, `test-sharp-${idx}.png`)
    await tile.toFile(filename)
    const { width: w, height: h } = await sharp(filename).metadata()
    expect(w).toBe(width)
    expect(h).toBe(size.height)
    fs.rmSync(filename)
  }))
})
...
test.concurrent('getUniqueHorizontalTiles', async () => {
  ...
  const tiles = await getUniqueHorizontalTiles(img, size, width, options)
...
test.skip.concurrent('getVerticalTiles', async () => {
...
test.skip.concurrent('getUniqueVerticalTiles', async () => {
  const img = await getImage(vertical.path)
  const size = await getSize(img)
  const height = size.height / 8
  const options: UniqueImagesOptions = {
...
test.skip.concurrent('getGridTiles', async () => {
...
test.skip.concurrent('getUniqueGridTiles', async () => {
  const img = await getImage(chess.path)
  const size = await getSize(img)
  const width = size.width / 8
  const height = size.height / 8
...
test.skip.concurrent('writeImages', { timeout: 50000 }, async () => {
...
describe.skip.concurrent('Test getBufferImages()', { timeout: 50000 }, () => {
